...
index = 250
jump = 15
plt.fill_between([x[index],x[index+jump]], [y[index], y[index]], lw=2, color='C1', zorder=5, label='Strook')
...
plt.annotate('',
             ...
             zorder=6)
plt.text(x[index]+1.2, -3.5, r'$2a-x$', fontsize=12, ha='center', va='bottom')
...
plt.annotate('',
             xy=(2*a+0.2, 0),
             xytext=(2*a+0.2, y[index]),
             arrowprops=dict(arrowstyle='<->', color='k',lw=1),
             zorder=6)
plt.text(2*a+.5, 1, r'$y$', fontsize=12, ha='center', va='bottom')
...
plt.savefig('functie_vraag8a.png', dpi=300)